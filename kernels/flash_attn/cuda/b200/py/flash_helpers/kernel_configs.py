...
# =============================================================================
# B200 (SM_100) specific kernel configs
# B200 has up to 228 KB shared memory per SM — 2.3× more than SM_120.
# This enables Bc=256, raising arithmetic intensity to ~32 FLOP/byte.
#
# smem = (Br + 2*Bc) * d_head * 2  bytes
#   Br=64,  Bc=256, d=128 → 147,456 B = 144 KB  < 228 KB ✓
#   Br=128, Bc=256, d=128 → 163,840 B = 160 KB  < 228 KB ✓
# =============================================================================

def get_b200_kernel_configs():
    """Return B200-optimized kernel configs with Bc=128 and Bc=256 tile sizes."""
    return [
        # --- Bc=128 configs (carry-over from SM_120 BL series, but on sm_100) ---
        # BL1: Bc=128 NW=4 stream(0,2,2)+opt_softmax
        FlashForwardKernelConfig(DType.FP16, 128, 64, 128, 4,
                                 True, True, True, 0, 2, 2, False, True),
        # BL2: Bc=128 NW=4 stream(0,2,2)+dbuf+opt_softmax
        FlashForwardKernelConfig(DType.FP16, 128, 64, 128, 4,
                                 True, True, True, 0, 2, 2, True, True),
        # BL3: Bc=128 NW=8 stream(0,2,2)+opt_softmax
        FlashForwardKernelConfig(DType.FP16, 128, 64, 128, 8,
                                 True, True, True, 0, 2, 2, False, True),
        # BL4: Br=Bc=128 NW=8 stream(2,2,2)+dbuf+opt_softmax
        FlashForwardKernelConfig(DType.FP16, 128, 128, 128, 8,
                                 True, True, True, 2, 2, 2, True, True),

        # --- Bc=256 configs — B200 EXCLUSIVE ---
        # B2_1: Br=64,  Bc=256, NW=8, stream(0,0,4) — V-dominated streaming
        FlashForwardKernelConfig(DType.FP16, 128, 64, 256, 8,
                                 True, True, True, 0, 0, 4, False, True),
        # B2_2: Br=64,  Bc=256, NW=8, stream(0,2,4) — K+V streaming
        FlashForwardKernelConfig(DType.FP16, 128, 64, 256, 8,
                                 True, True, True, 0, 2, 4, False, True),
        # B2_3: Br=128, Bc=256, NW=8, stream(0,2,2) — max tile, high AI
        FlashForwardKernelConfig(DType.FP16, 128, 128, 256, 8,
                                 True, True, True, 0, 2, 2, False, True),
        # B2_4: Br=128, Bc=256, NW=8, stream(2,2,2)+dbuf — full pipeline
        FlashForwardKernelConfig(DType.FP16, 128, 128, 256, 8,
                                 True, True, True, 2, 2, 2, True, True),
    ]


def get_wgmma_kernel_configs():
    """Return B200 WGMMA kernel configs (QK GEMM via wgmma.mma_async).
    Constraints: FP16 only, Bc in {64, 128}, n_warps multiple of 4,
    Br / (n_warps/4) == 64.
    """
    return [
        # KW_1: Br=64,  Bc=64,  NW=4 — 1 warpgroup, wgmma.m64n64k16
        FlashForwardKernelConfig(DType.FP16, 128, 64, 64, 4,
                                 True, True, True, 0, 2, 2, False, True),
        # KW_2: Br=64,  Bc=128, NW=4 — 1 warpgroup, wgmma.m64n128k16
        FlashForwardKernelConfig(DType.FP16, 128, 64, 128, 4,
                                 True, True, True, 0, 2, 2, False, True),
        # KW_3: Br=128, Bc=64,  NW=8 — 2 warpgroups, wgmma.m64n64k16
        FlashForwardKernelConfig(DType.FP16, 128, 128, 64, 8,
                                 True, True, True, 0, 2, 2, False, True),
        # KW_4: Br=128, Bc=128, NW=8 — 2 warpgroups, wgmma.m64n128k16
        FlashForwardKernelConfig(DType.FP16, 128, 128, 128, 8,
                                 True, True, True, 0, 2, 2, False, True),
    ]
