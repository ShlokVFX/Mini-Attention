...
// Include from the simplified header set (src_1-7_simplified/include/)
...
PYBIND11_MODULE(TORCH_EXTENSION_NAME, m) {
    m.def("forward", &flash_attention_forward,
          ...
          "Flash Attention forward (CUDA) — simplified header variant");

    // Set max dynamic smem on module load for kernels that exceed 48 KB.
