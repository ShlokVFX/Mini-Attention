...
def scaled_dot_product_attention(query, key, value, h_q, h_kv, is_causal=False):
    ...
    # PyTorch SDPA does not expose LSE directly
    lse = None
...
def run_torch_mla(q, block_table, blocked_k, max_seqlen_pad, block_size,
                  b, s_q, cache_seqlens, h_q, h_kv, d, dv, causal, dtype):

    blocked_v = blocked_k[..., :dv]

    def ref_mla():
        blocked_v = blocked_k[..., :dv]
        total_k = blocked_k.view(-1, h_kv, d)
        total_v = blocked_v.view(-1, h_kv, dv)
        ...
        for i in range(b):
            ...
            k = total_k[begin:end].transpose(0, 1)
            v = total_v[begin:end].transpose(0, 1)

            k = k.repeat_interleave(h_q // h_kv, dim=0)
            v = v.repeat_interleave(h_q // h_kv, dim=0)

            qi = q[i].transpose(0, 1).unsqueeze(0)
        ...
        return torch.cat(outputs, dim=0).transpose(1, 2), None

    out_torch, _ = ref_mla()
    t = triton.testing.do_bench(ref_mla)

    return out_torch, None, t
...
def compare_ab(baseline, target, b, s_q, cache_seqlens, h_q, h_kv, d, dv, causal, dtype):
    ...
    if lse_a is not None and lse_b is not None:
        torch.testing.assert_close(lse_b.float(), lse_a.float(), atol=1e-2, rtol=1e-2)
